...
async function main() {
    ...
    var createJoystickTab = true;
    if (createJoystickTab) {
        var button = document.createElement('button');
        button.setAttribute('class', 'tablinks');
        button.setAttribute('onclick', 'openDevice(event, "Joystick")');
        var node = document.createTextNode('Joystick');
        button.appendChild(node);
        document.getElementById('tabs').appendChild(button);

        //<div id="Pilotip" class="tabcontent">
        var div = document.createElement('div');
        div.setAttribute('id', 'Joystick');
        ...
        makeButton = function (id, i) {
            ...
            div.setAttribute('style', 'position:absolute; top:' + (325 + 75 * Math.floor((i - 1) / 6)) + 'px; left:' + (50 + ((i - 1) % 6) * 150) + 'px; z-index:2');
            ...
            input.setAttribute('style', 'text-align:center; font-size:10px; background-color:white');
            input.setAttribute('id', 'text' + id);
            input.setAttribute('value', 'NO DATA');
            input.readOnly = true;
            ...
            var tab = document.getElementById('Joystick');
            tab.appendChild(div);
        ...
        tcanvas.setAttribute('width', 25);
        tcanvas.setAttribute('height', 200);
        tcanvas.setAttribute('style', 'position:absolute; top:' + 500 + 'px; left:' + 350 + 'px; z-index:2; border: 1px solid black');
        tab.appendChild(tcanvas);

        jcanvas = document.createElement('canvas');
        jcanvas.setAttribute('id', 'jcanvas');
        jcanvas.setAttribute('width', 200);
        jcanvas.setAttribute('height', 200);
        jcanvas.setAttribute('style', 'position:absolute; top:' + 500 + 'px; left:' + 400 + 'px; z-index:2; border: 1px solid black');
        tab.appendChild(jcanvas);

        ycanvas = document.createElement('canvas');
        ycanvas.setAttribute('id', 'ycanvas');
        ycanvas.setAttribute('width', 87.5);
        ycanvas.setAttribute('height', 87.5);
        ycanvas.setAttribute('style', 'position:absolute; top:' + 500 + 'px; left:' + 625 + 'px; z-index:2; border: 1px solid black');
        tab.appendChild(ycanvas);

        hcanvas = document.createElement('canvas');
        hcanvas.setAttribute('id', 'hcanvas');
        hcanvas.setAttribute('width', 87.5);
        hcanvas.setAttribute('height', 87.5);
        hcanvas.setAttribute('style', 'position:absolute; top:' + 612.5 + 'px; left:' + 625 + 'px; z-index:2; border: 1px solid black');
        tab.appendChild(hcanvas);
    }
    ...
    var websocket = new WebSocket("ws://138.197.140.80/");
    ...
    websocket.onmessage = function (event) {
        if (event.data.text) {
            var promisedData = event.data.text();
            promisedData
                .then(function (successMessage) {
                    var message = tryParseJSON(successMessage);
                    if (!message) {
            ...
            if (!message) {
                ...
                try {
                    ...
                    reader.addEventListener('loadend', (e) => {
                        ...
                        var jsonMessage = tryParseJSON(text.split(' ').join(''));
                        if (!jsonMessage) {
                            console.log('Text unable to be read as JSON');
                        } else {
            ...
            } else if (isEmpty(message) || !message) {
                console.log('No data in topic');
    ...
    for (let key in json) {
        ...
        switch (key) {
            ...
            case 'CHT1':
                ...
                CHT_1_Bar.set(((value-150) % 100) / 100)
                break;
            case 'CHT2':
                updateText('Engine_CylinderHead2_Temp', value, value ? 'green' : 'red');
                CHT_2_Bar.set(((value-150) % 100) / 100)
                break;
